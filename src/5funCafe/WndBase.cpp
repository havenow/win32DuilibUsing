...
void CWndBase::OnFinalMessage(HWND hWnd)
{
	...
	if (m_bAddToMgr)
	{
		CWndMgr::Instance()->RemoveWnd(hWnd);
	}
	if (m_pShadowWnd)
	{
		delete m_pShadowWnd;
...
LRESULT CWndBase::OnCreate(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
{
	...
	if (m_bAddToMgr)
	{
		CWndMgr::Instance()->AddWnd(m_hWnd, this);
	}
	return 0;
}
...
LRESULT CWndBase::HandleMessage(UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	if (m_bShowShadow && uMsg == WM_PAINT)
	{
		if (NULL == m_pShadowWnd)
		{
			...
			m_bRoundRect ? m_pShadowWnd->SetBkImage(L"file='AeroRound.png' corner='16,15,16,19'") : m_pShadowWnd->SetBkImage(L"file='Aero.png' corner='11,10,11,14'");
			RECT rc = { 10, 9, 10, 13 };
